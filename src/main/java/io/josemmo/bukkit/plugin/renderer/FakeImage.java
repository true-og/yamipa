...
public class FakeImage extends FakeEntity {
     ...
     * @param flags    Flags
     */
    public FakeImage(@NotNull String filename, @NotNull Location location, @NotNull BlockFace face,
            @NotNull Rotation rotation, int width, int height, @Nullable Date placedAt, @NotNull OfflinePlayer placedBy,
            int flags)
    {
        ...
        // Define function for retrieving item frame positional vector from <row,column>
        // pair
    ...
    public boolean contains(@NotNull Location location, @NotNull BlockFace face) {
        ...
        if (point[0] < x[0] || point[0] > x[1])
            return false;
        if (point[1] < y[0] || point[1] > y[1])
            return false;
        if (point[2] < z[0] || point[2] > z[1])
            return false;
        return true;
     ...
     * <p>
     * Defines a single-use listener that gets called when the fake image finishes
     * loading, at which point the listener is automatically unregistered.
     * 
    ...
    private void load() {
        ...
        for (int col = 0; col < width; col++) {
            ...
            for (int row = 0; row < height; row++) {
                ...
                newFrames[height * col + row] = new FakeItemFrame(frameLocation, face, rotation, glowing,
                        maps[col][row]);
        ...
        if (animateImages && task == null && hasFlag(FLAG_ANIMATABLE) && numOfSteps > 1) {
            ...
            task = plugin.getScheduler().scheduleAtFixedRate(this::nextStep, 0, delay * 50L, TimeUnit.MILLISECONDS);
    ...
    public void spawn(@NotNull Player player) {
        ...
        plugin.fine(
                "Received request to spawn FakeImage#(" + location + "," + face + ") for Player#" + player.getName());
    ...
    public void destroy(@Nullable Player player) {
        ...
        plugin.fine("Received request to destroy FakeImage#(" + location + "," + face + ") for "
