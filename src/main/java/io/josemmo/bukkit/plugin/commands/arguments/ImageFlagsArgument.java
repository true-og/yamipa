...
public class ImageFlagsArgument extends Argument {
    ...
    private @NotNull CompletableFuture<Suggestions> getSuggestions(@NotNull CommandContext<?> ctx,
            @NotNull SuggestionsBuilder builder)
    {

        String input = builder.getRemaining().replaceAll("[^A-Z+\\-,]", "");
        int lastIndex = Collections
                .max(Arrays.asList(input.lastIndexOf(","), input.lastIndexOf("+"), input.lastIndexOf("-")));
    ...
    @Override
    public @NotNull Object parse(@NotNull CommandSender sender, @NotNull Object rawValue)
